#start lion-guard V1.0
json = dofile('./libs/JSON.lua');serpent = dofile("./libs/serpent.lua");local lgi = require ('lgi');local notify = lgi.require('Notify');notify.init ("Telegram updates");require('./libs/lua-redis');require('./bot/lionguard_Team');redis =  dofile("./libs/redis.lua");local minute = 60;local hour = 3600;local day = 86400;local week = 604800;TD_ID = redis:get('BOT-ID')
...
if redis:get('CheckBot:'..msg.chat_id) then
local text = '• Group `'..Company.title..'` ✪Hase Been Added To Group List✪ -// our channel==> @LionGuard'
...
local text = '• Group `'..Company.title..'` ✖Group Removed✖ '
 sendText(msg.chat_id, msg.id,text,'md')
else
...
SendMetion(msg.chat_id,Company.sender_user_id, msg.id, '• User : '..Company.sender_user_id..'✪ Added to manager List✪ ', 9,string.len(Company.sender_user_id))
else
SendMetion(msg.chat_id,Company.sender_user_id, msg.id, '• User : '..Company.sender_user_id..' ✪ Added to manager List✪ ', 9,string.len(Company.sender_user_id))
...
if redis:sismember('OwnerList:'..msg.chat_id,user) then
SendMetion(msg.chat_id,user, msg.id, '• User : '..user..' ✪ Added to manager List✪', 9,string.len(user))
else
SendMetion(msg.chat_id,user, msg.id, '• User : '..user..' ✪ Added to manager List✪', 9,string.len(user))
...
SendMetion(msg.chat_id,Company.id, msg.id, '• User : '..Company.id..' ✪ Added to manager List✪', 9,string.len(Company.id))
else
SendMetion(msg.chat_id,Company.id, msg.id, '• User : '..Company.id..' ✪ Added to manager List✪', 9,string.len(Company.id))
...
sendText(msg.chat_id, msg.id,  'متاسفم :( \nمن نمیتوانم پیام خودم را چک کنم', 'md')
return false
end
if redis:sismember('GlobalyBanned:',user) then
...
sendText(msg.chat_id,msg.id," ⇋ تمام مدیران گروه به رسمیت شناخته شدند ", "md")
end
...
sendText(msg.chat_id, msg.id,"• _Done_ \n*User* `"..locks.."` Unblock Member ✔ *\nRestricted*",  'md' )
 ...
 local user = Company.sender_user_id
sendText(msg.chat_id, msg.id, '• User '..(user or 00000000)..' ⇋با موفقیت تنظیم شد','md')
end
...
sendText(msg.chat_id, msg.id, '• User `'..(Company.sender_user_id or 00000000)..'` ⇋ادمین با موفقیت حذف شد ', 'md')
end
if tonumber(msg.reply_to_message_id) == 0 then
else
...
sendText(msg.chat_id,msg.reply_to_message_id, '•  Has Been Pinned' ,'md')
...
sendText(msg.chat_id, msg.id,'⇋قفل همگانی فعال شد ' ,'md')
end
...
redis:del('ModList:'..msg.chat_id)
sendText(msg.chat_id, msg.id,  ' ⇋لیست ادمین ها با موفقیت پاکسازی شد ', 'md')
end
----
end
----
...
sendText(msg.chat_id, msg.id, '• ALL Message  `'..(Company.sender_user_id or 00000000)..'`✔تمامی پیام ها پاک شد', 'md')
...
sendText(msg.chat_id, msg.id,  '⇋لیست اخراجی ها با موفقیت پاکسازی شد ', 'md')
...
sendText(msg.chat_id, msg.id,  '⇋لیست افراد سایلنت با موفقیت پاکسازی شد ', 'md')
end
...
sendText(msg.chat_id, msg.id,  '⇋تمامی ربات ها اخراج شدند ', 'md')
...
sendText(msg.chat_id, msg.id,  '• `User : ` *'..(Company.sender_user_id or 00000000)..'✔به لیست افراد ویژه افزوده شد`', 'md')
else
...
sendText(msg.chat_id, msg.id,  '⇋لیست افراد ویژه با موفقیت پاکسازی شد', 'md')
end
if energy == 'remvip' then
function RemVipByReply(energy,Company)
if redis:sismember('Vip:'..msg.chat_id, Company.sender_user_id or 00000000) then
sendText(msg.chat_id, msg.id,'• _User : _ `'..(Company.sender_user_id or 00000000)..'`فرد ویژه ازلیست حذف شد', 'md')
redis:srem('Vip:'..msg.chat_id, Company.sender_user_id or 00000000)
else
sendText(msg.chat_id, msg.id,  '• `User : ` *'..(Company.sender_user_id or 00000000)..'در لیست افراد ویژه قرار ندارد`', 'md')
end
end
...
sendPhoto(msg.chat_id, msg.id, 0, 1, nil, Company.photos[0].sizes[2].photo.persistent_id,'• ʟɪᴏɴɢᴜᴀʀᴅ\n\nChat ID : ['..msg.chat_id..']\nUser ID : ['..msg.sender_user_id..']\nRank : ['..rank..']\nTotal Msgs : '..Msgs..'\nTD ID : '..TD_ID..'\n')
else
sendText(msg.chat_id, msg.id,  '• `ʟɪᴏɴɢᴜᴀʀᴅ`!!\n\nChat ID : ['..msg.chat_id..']\nUser ID : ['..msg.sender_user_id..']\nRank : ['..rank..']\nTotal Msgs : '..Msgs..'\nTD ID : '..TD_ID..'\n', 'md')
print '                      Not Photo                      ' 
end
end
...
showedit(amir,data)
end;assert (tdbot_function ({_ = "getMessage", chat_id = data.chat_id,message_id = data.message_id }, edit, nil));assert (tdbot_function ({_ = "openChat",chat_id = data.chat_id}, dl_cb, nil) );assert (tdbot_function ({ _ = 'openMessageContent',chat_id = data.chat_id,message_id = data.message_id}, dl_cb, nil));require('./bot/lionguard_Team');assert (tdbot_function ({_="getChats",offset_order="9223372036854775807",offset_chat_id=0,limit=20}, dl_cb, nil));end;end
---
